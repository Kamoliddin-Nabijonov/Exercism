...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    is_criticality_balanced = False
    if temperature < 800 and neutrons_emitted > 500 and (temperature * neutrons_emitted < 500000):
        is_criticality_balanced = True
    return is_criticality_balanced
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    status_value = temperature * neutrons_produced_per_second
    status_code = None

    if status_value < (threshold/100 * 90):
        status_code = 'LOW'
    elif (threshold - (threshold/100 * 10)) < status_value < (threshold + (threshold/100 * 10)):
        status_code = 'NORMAL'
    else:
        status_code = 'DANGER'
